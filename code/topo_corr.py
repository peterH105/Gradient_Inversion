...
def topo_corr(lon,lat,bed,ice,heights,point_mass_number):
    ...
    J=create_Jacobian(lon,lat,bed_top*-1,bed_bottom*-1,heights, density_topo, point_mass_number)
    ...
    if np.any(ice_above_top) or np.any(ice_above_bottom):
        ...
        J=create_Jacobian(lon,lat,ice_above_top*-1,ice_above_bottom*-1,heights, density_ice_above, point_mass_number)
    ...
    if np.any(ice_below_top) or np.any(ice_below_bottom):
        ...
        J=create_Jacobian(lon,lat,ice_below_top*-1,ice_below_bottom*-1,heights, density_ice_below, point_mass_number)
